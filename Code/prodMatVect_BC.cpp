   ...
   if (proc.me == 0){
      ...
      for (int j = 0; j < proc.nbElem_y; j++)
      {
         for (int i = 0; i < dom.Nx; i++)
         {
            ...
            if (j == proc.nbElem_y - 1)
            {
               U[I] += nu * (stencilOver[i + (proc.nbElem_y-dom.r-2)*dom.Nx] + stencilOver[i + (proc.nbElem_y-dom.r)*dom.Nx] ) + lambda * stencilOver[i + (proc.nbElem_y-dom.r-1) * dom.Nx];
            }
            if (i == 0)
            {
               U[I] -= b * h(0, j + 1, t + dom.dt, dom);
            }
            if (i == dom.Nx - 1)
            {
               U[I] -= b * h(dom.Nx + 1, j + 1, t + dom.dt, dom);
            }
         }
      }

   } else if ( proc.me == proc.nbProc-1 ){
      ...
      for (int j = 0; j < proc.nbElem_y; j++)
      {
         for (int i = 0; i < dom.Nx; i++)
         {
            ...
            if (j == 0)
            {
               U[I] += nu * (stencilUnder[i + (dom.r-1)*dom.Nx] + stencilUnder [ i + (dom.r+1)*dom.Nx] ) + lambda * stencilUnder[i + dom.r * dom.Nx];
            }
            if (j == proc.nbElem_y - 1)
            {
               U[I] -= c * g(i + 1, proc.nbElem_y + 1, t + dom.dt, dom);
            }
            if (i == 0)
            {
               U[I] -= b * h(0, j + 1, t + dom.dt, dom);
