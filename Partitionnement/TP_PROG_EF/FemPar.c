...
void Update(float* A)
{
   ...
   for (int i = 0; i < ProcNo; i++)
   {
      ...
      if (Common[i] > 0)
      {
         //MPI_Send(&Buf[0], Common[i], MPI_FLOAT, i, 100, MPI_COMM_WORLD);
         MPI_Send(&Buf[0], taille * Common[i], MPI_FLOAT, i, 100, MPI_COMM_WORLD);
      }
   }
   for (int i = 0; i < ProcNo; i++)
   {
      if (Common[i] > 0)
      {
         //MPI_Recv(&Buf[0], Common[i], MPI_FLOAT, i, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Recv(&Buf[0], taille * Common[i], MPI_FLOAT, i, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
      for (int j = 0; j < Common[i]; j++)
      {
...
int main(int argc, char** argv)
{
   ...
   char FileName[40], OutName[40], timeFilename[100];
   int I, J, K, II, JJ;
   float** Ap, ** Bp, ** As, * Fp, * Fs, * Vp, * Vs;
   float SLoc[3][2], TwoA, Area, Gr0[3], Gr1[3], StiffJI;

   if (argc < 2) {
      printf("Please specify a folder name.\n");
      return 1;
   }
   myFolder = argv[1];
   ...
   double mytime = MPI_Wtime(), maxtime;
   ...
   // ça fonctionne   Buf = (float*)malloc(MaxCommon * sizeof(float));
   Buf = (float*)calloc(taille * MaxCommon, sizeof(float));
   fclose(InFile);
   ...
   if (ProcID == 0)
   {
      strcpy(timeFilename, myFolder);
      strcat(timeFilename, "time.dat");

      printf("Le chemin complet est : %s\n", timeFilename);
